...
import { parseWindow, subtractIntervals, durationMinutesOf } from "./windows.js";
 ...
 */
export function isBookable(window: TimeWindow, existingBookings: TimeWindow[], requiredMinutes: number): boolean {
  const base = parseWindow(window);
  const busy = existingBookings.map(parseWindow);
  const free = subtractIntervals(base, busy);

  return free.some((interval) => durationMinutesOf(interval) >= requiredMinutes);
}
