...
export function parseWindow(window: TimeWindow): ParsedWindow {
  ...
  if (Number.isNaN(startMs) || Number.isNaN(endMs)) {
    throw new Error(`Invalid TimeWindow: start=${window.start} end=${window.end}`);
  }
  if (endMs <= startMs) {
    throw new Error(`Invalid TimeWindow: end (${window.end}) must be after start (${window.start})`);
  }
...
export function intersect(a: ParsedWindow, b: ParsedWindow): ParsedWindow | null {
...
export function subtractIntervals(base: ParsedWindow, busy: ParsedWindow[]): ParsedWindow[] {
