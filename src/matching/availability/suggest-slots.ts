...
import { intersect, parseWindow, subtractIntervals, durationMinutesOf, toTimeWindow } from "./windows.js";
    ...
    for (const v of parsedVolunteer) {
      ...
      for (const segment of freeSegments) {
        if (durationMinutesOf(segment) >= requiredMinutes) {
          slots.push({ startMs: segment.startMs, endMs: segment.startMs + requiredMinutes * 60_000 });
        }
      }
    }
  }
