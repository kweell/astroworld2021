import type { MatchRequest, MatchResult, VolunteerCandidate } from "../types/volunteer.js";
...
function scoreCandidate(request: MatchRequest, candidate: VolunteerCandidate) {
  ...
  const expertiseMatches = tagOverlap(request.topicTags, candidate.expertiseTags);
  const expertiseScore = overlapRatio(request.topicTags, candidate.expertiseTags) * WEIGHTS.expertise;
  score += expertiseScore;
  if (expertiseMatches.length > 0) {
    reasons.push(`Strong match for ${expertiseMatches.join(", ")}`);
  }

  const industryMatches = tagOverlap(request.industryTags, candidate.industryTags);
  const industryScore = overlapRatio(request.industryTags, candidate.industryTags) * WEIGHTS.industry;
  score += industryScore;
  if (industryMatches.length > 0) {
    reasons.push(`Experience in ${industryMatches.join(", ")}`);
  }

  const needsSchedule = filterResult.liveRequired || request.preferredMode === "either";
  ...
  if (!needsSchedule) {
    ...
    reasons.push(`Available for a ${request.durationMinutes}-minute slot on ${day}`);
  } else {
    availabilityScore = 0;
  }
  ...
  const languageScore = overlapRatio(request.languages, candidate.languages) * WEIGHTS.language;
  score += languageScore;
  ...
  if (request.accessPreferences.length === 0) {
    ...
    const accessMatches = tagOverlap(request.accessPreferences, candidate.supportedAccessPreferences);
    accessScore = (accessMatches.length / request.accessPreferences.length) * WEIGHTS.access;
  ...
  if (livedExperiencePreferences.length > 0) {
    const livedMatches = tagOverlap(livedExperiencePreferences, candidate.livedExperienceTags);
    livedExperienceScore = (livedMatches.length / livedExperiencePreferences.length) * WEIGHTS.livedExperience;
