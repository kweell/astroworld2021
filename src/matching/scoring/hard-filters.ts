 ...
 */
export function applyHardFilters(request: MatchRequest, candidate: VolunteerCandidate): HardFilterResult {
  ...
  if (!acceptableModes.some((mode) => candidate.supportedModes.includes(mode))) {
    failureReasons.push("Volunteer does not support the requested interaction mode");
  }

  if (request.accessPreferences.length > 0) {
    const supported = tagOverlap(request.accessPreferences, candidate.supportedAccessPreferences);
    if (supported.length === 0) {
      failureReasons.push("Volunteer cannot support the participant's access requirements");
    }
  }
  ...
  const liveRequired =
    ...
    (request.preferredMode === "either" && !candidate.supportedModes.includes("async"));

  let compatibleWindows: TimeWindow[] = [];
  if (liveRequired) {
    compatibleWindows = findOverlap(request.availabilityWindows, candidate.availableWindows, request.durationMinutes);
    if (compatibleWindows.length === 0) {
      failureReasons.push("No sufficient availability overlap for a live interaction");
    }
  ...
  return { eligible: failureReasons.length === 0, failureReasons, liveRequired, compatibleWindows };
}
