...
import { askMeAnythingRequest, teachMeSomethingRequest, reviewMyWorkRequest } from "../fixtures/requests.js";
...
describe("matchVolunteers", () => {
  it("ranks the best expertise match first", () => {
    ...
    expect(results[0].reasons.some((r) => r.includes("cybersecurity"))).toBe(true);
  });
  ...
  it("lets language overlap affect the score", () => {
    const base: MatchRequest = { ...askMeAnythingRequest, accessPreferences: [] };
    ...
    const withLanguage = matchVolunteers(base, [{ ...candidate, languages: ["english"] }])[0];
  ...
  it("lets access requirement compatibility affect eligibility and score", () => {
    const request: MatchRequest = { ...askMeAnythingRequest, accessPreferences: ["written_instructions"] };
    const candidate: VolunteerCandidate = {
      volunteerId: "vol-y",
      supportedServices: ["ask_me_anything"],
      expertiseTags: ["cybersecurity"],
    ...
    const supported = { ...candidate, supportedAccessPreferences: ["written_instructions"] };
    const results = matchVolunteers(request, [supported]);
    expect(results[0].reasons.some((r) => r.includes("written_instructions"))).toBe(true);
  });
  ...
  it("breaks ties deterministically by ascending volunteerId", () => {
    ...
    const request: MatchRequest = { ...askMeAnythingRequest, accessPreferences: [], livedExperiencePreferences: [] };
    const results = matchVolunteers(request, [identical("vol-zeta"), identical("vol-aardvark")]);
    expect(results.map((r) => r.volunteerId)).toEqual(["vol-aardvark", "vol-zeta"]);
  });
  ...
  it("returns at most 5 candidates by default, or options.limit", () => {
    ...
    const limited = matchVolunteers(askMeAnythingRequest, volunteers, { limit: 1 });
