...
const TUE_09_11_SGT = { start: "2025-03-04T01:00:00Z", end: "2025-03-04T03:00:00Z" };
const TUE_10_12_SGT = { start: "2025-03-04T02:00:00Z", end: "2025-03-04T04:00:00Z" };
...
describe("findOverlap", () => {
  it("returns the overlapping window when both sides have enough overlap", () => {
    ...
    expect(result).toEqual([{ start: "2025-03-04T02:00:00.000Z", end: "2025-03-04T03:00:00.000Z" }]);
  });
  ...
  it("handles multiple windows on each side and returns them sorted chronologically", () => {
    ...
    expect(result.map((w) => w.start)).toEqual(["2025-03-04T01:30:00.000Z", "2025-03-05T04:30:00.000Z"]);
  });

  it("throws on an invalid window where end is not after start", () => {
    expect(() => findOverlap([{ start: "2025-03-04T01:00:00Z", end: "2025-03-04T01:00:00Z" }], [TUE_10_12_SGT], 10)).toThrow();
  });
});

describe("isBookable", () => {
  it("is true when a window has enough free time after removing existing bookings", () => {
    const existing = [{ start: "2025-03-04T01:00:00Z", end: "2025-03-04T01:30:00Z" }];
    expect(isBookable(TUE_09_11_SGT, existing, 30)).toBe(true);
  });

  it("prevents double-booking when the window is fully consumed", () => {
    const existing = [{ start: "2025-03-04T01:00:00Z", end: "2025-03-04T03:00:00Z" }];
    expect(isBookable(TUE_09_11_SGT, existing, 30)).toBe(false);
  });

  it("is false when remaining free time is shorter than required", () => {
    const existing = [{ start: "2025-03-04T01:00:00Z", end: "2025-03-04T02:50:00Z" }];
    expect(isBookable(TUE_09_11_SGT, existing, 30)).toBe(false);
  });
});
...
describe("suggestSlots", () => {
  it("suggests a slot inside the participant/volunteer overlap", () => {
    ...
    expect(slots).toEqual([{ start: "2025-03-04T02:00:00.000Z", end: "2025-03-04T02:30:00.000Z" }]);
  });
  ...
  it("does not suggest a slot that collides with an existing booking", () => {
    ...
    const participant = { start: "2025-03-04T01:00:00Z", end: "2025-03-04T04:00:00Z" };
    const volunteer = { start: "2025-03-04T02:00:00Z", end: "2025-03-04T04:30:00Z" };
    const existing = [{ start: "2025-03-04T02:00:00Z", end: "2025-03-04T02:45:00Z" }];
    const slots = suggestSlots([participant], [volunteer], existing, 30);
    expect(slots).toEqual([{ start: "2025-03-04T02:45:00.000Z", end: "2025-03-04T03:15:00.000Z" }]);
  });
