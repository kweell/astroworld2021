...
import { askMeAnythingRequest, teachMeSomethingRequest } from "../fixtures/requests.js";
...
describe("applyHardFilters", () => {
  it("filters out an unverified volunteer", () => {
    const result = applyHardFilters(teachMeSomethingRequest, findVolunteer("vol-chen"));
  ...
  it("filters out a volunteer who does not support the requested service", () => {
    const result = applyHardFilters(teachMeSomethingRequest, findVolunteer("vol-devi"));
    expect(result.eligible).toBe(false);
    expect(result.failureReasons.some((r) => r.includes("does not support"))).toBe(true);
  });

  it("filters out a volunteer without enough remaining capacity", () => {
    const result = applyHardFilters(askMeAnythingRequest, findVolunteer("vol-erin"));
    expect(result.eligible).toBe(false);
    expect(result.failureReasons).toContain("Volunteer does not have enough remaining capacity");
  });

  it("filters out a volunteer with no sufficient live availability overlap", () => {
    const result = applyHardFilters(teachMeSomethingRequest, findVolunteer("vol-ivan"));
    expect(result.eligible).toBe(false);
    expect(result.failureReasons).toContain("No sufficient availability overlap for a live interaction");
  });

  it("does not require availability overlap for an async request", () => {
    const result = applyHardFilters(askMeAnythingRequest, findVolunteer("vol-gopal"));
  ...
  it("filters out a volunteer who cannot support any declared access requirement", () => {
    ...
    expect(result.eligible).toBe(false);
    expect(result.failureReasons.some((r) => r.includes("access requirements"))).toBe(true);
  });
